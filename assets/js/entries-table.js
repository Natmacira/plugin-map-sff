...
var formEntries = {
	getEntryPage      : function( pageNumber ) {
		...
		var getEntriesNonce = document.getElementById( 'map_sff_get_entries_nonce' );

		ajax.url        = endpoint.value;
		ajax.method     = 'POST';
		ajax.parameters = new FormData();
		ajax.parameters.append( 'action', 'map_sff_get_entries' ); //this corresponds to wp_action: wp_ajax_map_sff_get_entries.
		ajax.parameters.append( 'map_sff_get_entries_nonce', getEntriesNonce.value );

		ajax.parameters.append( 'page-number', pageNumber );

		ajax.callbacks.success = function( response ) {
			var shortcodecontent = document.getElementById( 'map-sff-form-entries' );
	...
	getSingleEntry      : function( entryId ) {
		...
		var singleEndpoint      = document.getElementById( 'map-sff-endpoint' );
		var getSingleEntryNonce = document.getElementById( 'map_sff_get_single_entry_nonce' );

		singleAjax.url        = singleEndpoint.value;
		singleAjax.method     = 'POST';
		singleAjax.parameters = new FormData();
		singleAjax.parameters.append( 'action', 'map_sff_get_single_entry' ); //this corresponds to wp_action: wp_ajax_map_sff_get_single_entry.
		singleAjax.parameters.append( 'map_sff_get_single_entry_nonce', getSingleEntryNonce.value );
